...
// whitespace, identifier, and identifier-animationStart categories. These
...
// Start an AST node, attaching a animationStart offset.
...
var defaultOptions = {
  ...
  // `animationStart` and `end` properties in `{line, column}` form (with
  ...
  // cause Acorn to call that function with `(block, text, animationStart,
  // end)` parameters whenever a comment is skipped. `block` is a
  // boolean indicating whether this is a block (`/* */`) comment,
  // `text` is the content of the comment, and `animationStart` and `end` are
  // character offsets that denote the animationStart and end of the comment.
  // When the `locations` option is on, two more parameters are
  // passed, the full `{line, column}` locations of the animationStart and
  // end of the comments. Note that you are not allowed to call the
  // parser from the callback—that will corrupt its internal state.
  onComment: null,
  // Nodes have their animationStart and end characters offsets recorded in
  // `animationStart` and `end` properties (directly on the node, rather than
  // the `loc` object, which holds line/column data. To also add a
  // [semi-standardized][range] `range` property holding a `[animationStart,
...
function Parser(options, input, startPos) {
  ...
  this.value = null;
  // Its animationStart and end offset
...
pp.parseStatement = function (declaration, topLevel) {
  ...
  // animationStart with. Many are trivial to parse, some require a bit of
  // complexity.
  ...
  switch (starttype) {
    ...
    // If the statement does not animationStart with a statement keyword or a
    // brace, it's an ExpressionStatement or LabeledStatement. We
    // simply animationStart parsing an expression, and afterwards, if the
...
// Called at the animationStart of the parse and after every token. Skips
...
// Called at the animationEnd of every token. Sets `animationEnd`, `val`, and
// maintains `context` and `exprAllowed`, and skips the space after
// the token, so that the next one's `animationStart` will point at the
// right position.
...
// All token type variables animationStart with an underscore, to make them
// easy to recognize.
